 ...
 * 
 * @author: Jiawei Wu
 * of the quagga 0.99.7/src/opapd/opap_pap.c code which was ported here
 */
...
void
RLRoutingDB::Initialize (int *adjacencyArray, NodeContainer nodes)
{
  NS_LOG_FUNCTION (this);
  // 计算节点数目
...
void
RLRoutingDB::SetWeightMatrix (double *weightArray, uint32_t nodeNum)
{
  NS_LOG_FUNCTION (this);
...
int
RLRoutingDB::IsAdjacency (Edge edge)
{
  return m_adjacencyMatrix[edge];
}

int
RLRoutingDB::IsAdjacency (uint32_t src, uint32_t dst)
{
...
int
RLRoutingDB::IsReachable (Edge edge)
{
  return m_reachableMatrix[edge];
}

int
RLRoutingDB::IsReachable (uint32_t src, uint32_t dst)
{
  Edge edge = Edge (src, dst);
...
int
RLRoutingDB::IsValidPath (NodeId src, NodeId next, NodeId dst)
{
...
void
RLRoutingDB::SetAdjacencyMatrix (int *adjacencyArray, uint32_t nodeNum)
{
  NS_LOG_FUNCTION (this);
  for (uint32_t src = 0; src < nodeNum; src++)
    {
...
void
RLRoutingDB::CalcReachableMatrix (int *adjacencyArray, uint32_t nodeNum)
{
...
void
RLRoutingDB::InitNextNodeMatrix (NodeContainer nodes)
{
  // 初始化oif映射关系
  NS_LOG_FUNCTION (this);
...
void
RLRoutingDB::InitWeightMatrix (uint32_t nodeNum)
{
  NS_LOG_FUNCTION (this);
  // 遍历所有发包节点
  for (uint32_t src = 0; src < nodeNum; src++)
...
void
RLRouteManagerImpl::DeleteRoutes ()
{
  NS_LOG_FUNCTION (this);
...
//
void
RLRouteManagerImpl::BuildRLRoutingDatabase (int *adjacencyArray, NodeContainer nodes)
{
  NS_LOG_FUNCTION (this);
  m_nodes = nodes;
...
// 设置权重矩阵
void
RLRouteManagerImpl::SetWeightMatrix (double *weightArray)
{
  NS_LOG_FUNCTION (this);
...
// 将其写入src的路由表
void
RLRouteManagerImpl::CalculateRoutes ()
{
  NS_LOG_FUNCTION (this);
  // 遍历所有节点
  uint32_t nodeNum = m_nodes.GetN ();
