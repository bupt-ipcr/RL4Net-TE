...
Ptr<FlowMonitor>
FlowMonitorHelper::RLInstall (Ptr<Node> node)
{
  Ptr<FlowMonitor> monitor = GetMonitor ();
  Ptr<FlowClassifier> classifier = GetClassifier ();
  Ptr<Ipv4L3Protocol> ipv4 = node->GetObject<Ipv4L3Protocol> ();
  if (ipv4)
    {
      Ptr<RLFlowProbe> probe = Create<RLFlowProbe> (monitor,
                                                        DynamicCast<Ipv4FlowClassifier> (classifier),
                                                        node);
    }
  return m_flowMonitor;
...
Ptr<FlowMonitor>
FlowMonitorHelper::RLInstall (NodeContainer nodes)
{
  for (NodeContainer::Iterator i = nodes.Begin (); i != nodes.End (); ++i)
    {
      Ptr<Node> node = *i;
...
Ptr<FlowMonitor>
FlowMonitorHelper::RLInstallAll ()
{
  for (NodeList::Iterator i = NodeList::Begin (); i != NodeList::End (); ++i)
    {
      Ptr<Node> node = *i;
