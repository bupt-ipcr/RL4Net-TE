...
public:
  ...
  /// 重载
  void ReportFirstTx (Ptr<RLFlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t nodeId, uint32_t interface);
  /// FlowProbe implementations are supposed to call this method to
  ...
  /// 重载
  void ReportForwarding (Ptr<RLFlowProbe> probe, uint32_t flowId, uint32_t packetId, uint32_t packetSize, uint32_t nodeId, uint32_t interface);
